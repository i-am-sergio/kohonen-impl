...
private:
    ...
    {
        if (s_instance)
            s_instance->actualDisplay();
    }
    static void reshapeCallback(int width, int height)
    {
        if (s_instance)
            s_instance->actualReshape(width, height);
    }
    static void mouseMotionCallback(int x, int y)
    {
        if (s_instance)
            s_instance->actualMouseMotion(x, y);
    }
    static void mouseButtonCallback(int button, int state, int x, int y)
    {
        if (s_instance)
            s_instance->actualMouseButton(button, state, x, y);
    }
    static void keyboardCallback(unsigned char key, int x, int y)
    {
        if (s_instance)
            s_instance->actualKeyboard(key, x, y);
    }
    static void specialKeyboardCallback(int key, int x, int y)
    { // Nuevo callback estático
        if (s_instance)
            s_instance->actualSpecialKeyboard(key, x, y);
    }
};
